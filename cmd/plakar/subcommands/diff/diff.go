...
func diff_files(ctx *appcontext.AppContext, snap1 *snapshot.Snapshot, fileEntry1 *vfs.Entry, snap2 *snapshot.Snapshot, fileEntry2 *vfs.Entry) (string, error) {
	if fileEntry1.Object == fileEntry2.Object {
		fmt.Fprintf(ctx.Stderr, "%s:%s and %s:%s are identical\n",
			fmt.Sprintf("%x", snap1.Header.GetIndexShortID()), path.Join(fileEntry1.ParentPath, utils.EscapeANSICodes(fileEntry1.Stat().Name())),
			fmt.Sprintf("%x", snap2.Header.GetIndexShortID()), path.Join(fileEntry2.ParentPath, utils.EscapeANSICodes(fileEntry2.Stat().Name())))
		return "", nil
	}
	...
	diff := difflib.UnifiedDiff{
		...
		FromFile: fmt.Sprintf("%x", snap1.Header.GetIndexShortID()) + ":" + utils.EscapeANSICodes(filename1),
		ToFile:   fmt.Sprintf("%x", snap2.Header.GetIndexShortID()) + ":" + utils.EscapeANSICodes(filename2),
		Context:  3,
	}
